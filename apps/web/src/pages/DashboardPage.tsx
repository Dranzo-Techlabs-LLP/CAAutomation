    ...
    <section className="space-y-5 p-4 lg:p-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <span className="hidden rounded-lg border border-border bg-panel px-3 py-1.5 text-xs font-medium text-muted-foreground sm:inline">
            {now.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
          </span>
          <select value={period} onChange={(e) => setPeriod(e.target.value as Period)} className="input-field h-auto w-auto py-1.5 text-xs font-medium">
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        <div className="panel lg:col-span-5">
          ...
          {topStaff.length === 0 ? <Empty text={canViewReports ? 'No data' : 'Reporting access required'} /> : (
            <table className="w-full text-left text-[13px]">
              <thead className="text-[10.5px] uppercase text-muted-foreground"><tr><th className="py-1.5 font-medium">#</th><th className="font-medium">Staff</th><th className="text-right font-medium">Revenue</th><th className="text-right font-medium">Hours</th><th className="text-right font-medium">Util</th><th className="text-right font-medium">Realization</th></tr></thead>
              <tbody className="divide-y divide-border">
                {topStaff.map((s, i) => (
                  <tr key={s.userId}>
                    <td className="py-2 text-muted-foreground">{i + 1}</td>
                    <td className="truncate font-medium">{s.userName}</td>
                    <td className="text-right font-mono tabular-nums">{fmtP(s.revenuePaise)}</td>
                    <td className="text-right tabular-nums text-muted-foreground">{(s.totalMinutes / 60).toFixed(2)}</td>
                    <td className="text-right tabular-nums text-muted-foreground">{s.utilizationPct}%</td>
                    <td className="text-right font-mono tabular-nums text-muted-foreground">{fmtP(s.ratePerHourPaise)}/hr</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="panel lg:col-span-4">
          ...
          {topClients.length === 0 ? <Empty text={canViewReports ? 'No data' : 'Reporting access required'} /> : (
            <table className="w-full text-left text-[13px]">
              <thead className="text-[10.5px] uppercase text-muted-foreground"><tr><th className="py-1.5 font-medium">#</th><th className="font-medium">Client</th><th className="text-right font-medium">Revenue</th><th className="text-right font-medium">Outstanding</th><th className="text-right font-medium">Profit.</th></tr></thead>
              <tbody className="divide-y divide-border">
                {topClients.map((c, i) => (
                  <tr key={c.customerId}>
                    <td className="py-2 text-muted-foreground">{i + 1}</td>
                    <td className="truncate font-medium" title={c.customerName}>{c.customerName}</td>
                    <td className="text-right font-mono tabular-nums">{fmtP(c.revenuePaise)}</td>
                    <td className="text-right font-mono tabular-nums text-muted-foreground">{fmtP(c.outstandingPaise)}</td>
                    <td className="text-right tabular-nums">
                      <span className={`inline-flex items-center gap-1 ${c.marginPct >= 50 ? 'text-emerald-600' : c.marginPct >= 0 ? 'text-amber-600' : 'text-red-600'}`}>
                        <span className="h-1.5 w-1.5 rounded-full" style={{ background: 'currentColor' }} />{c.marginPct}%
                      </span>
                    </td>
                  </tr>
                ))}
...
function Kpi({ label, value, sub, icon, tone, delta, deltaSuffix, spark, alert }: {
  ...
  return (
    <div className={`rounded-xl border bg-panel p-4 shadow-sm ${alert ? t.ring + ' ' + (t.bg && 'bg-red-50/50 dark:bg-red-900/10') : 'border-border'}`}>
      ...
      <p className={`mt-1.5 text-2xl font-bold tracking-tight tabular-nums ${alert ? 'text-red-600' : ''}`}>{value}</p>
      <div className="mt-0.5 flex items-center gap-1.5">
        ...
        {(sub || deltaSuffix) && <span className="truncate text-[10.5px] text-muted-foreground">{delta != null ? deltaSuffix : sub}</span>}
      </div>
...
function Stat({ label, value, sub, delta, deltaSuffix, tone }: { label: string; value: string; sub?: string; delta?: number | null; deltaSuffix?: string; tone?: string }) {
  return (
    <div>
      ...
      <p className={`mt-0.5 text-lg font-bold tabular-nums ${tone === 'emerald' ? 'text-emerald-600' : ''}`}>{value}</p>
      {delta != null ? (
        <span className={`inline-flex items-center gap-0.5 text-[10.5px] font-semibold ${delta >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
